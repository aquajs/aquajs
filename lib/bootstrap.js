...
module.exports = function (config) {
  ...
  if(logConfig.logConfig.precog != undefined){
    initPrecogLgger(app,config.app.logconfpath,AquaJsLogger.getWinston(),$logger);
  }

  require('./express')(app);
 if(!$loadModel && fs.existsSync($dirPaths.modelsDir)){
...
function initSwagger(app) {
  ...
  var p = path.join(process.cwd(), 'node_modules', 'aquajs-swagger-ui', 'dist');
  ...
  app.get(/^\/apidocSample(\/.*)?$/, function (req, res, next) {
    if (req.url === '/apidocSample') {
      var sampleData = rekuire(path.join($dirPaths.serverDir,'data','sampleData'));
