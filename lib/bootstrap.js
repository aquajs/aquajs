...
var express = require('express'),
  ...
  async = require("async"),
  aquaError = require('aquajs-error'),
        ...
        if (eachConfig["adapters"] !== undefined) {
          orm.initialize(eachConfig, function (err, models) {
            try {
              ...
              throw new aquaError("error: ensure mongo is running before starting microservice");
            }
      ...
      if (enablePersist) {
        if (eachConfig["driver"] !== undefined) {
          persist.connect(eachConfig, function (err, conn) {
            if (err) {
              throw new aquaError("oracle connection could not be established");
            }
            GLOBAL.$conn = conn;
          });
        }
