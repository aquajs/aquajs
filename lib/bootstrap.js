...
var express = require('express'),
    ...
    connUtils = require('./../util/connUtils'),
    monitor = require('./monitor');
    require('./../util/globalConfig');
...
module.exports = function (config) {
  ...
  if (config.app.loadModel != undefined && config.app.loadModel && !$initModel && fs.existsSync($dirPaths.modelsDir)) {
    enableWaterline=(config.dbType==='orm-nosql')?true:false;
    enablePersist =(config.dbType==='orm-sql')?true:false;
    connUtils.initORM(config.app.dbConfList, app);
  }
